...
class Model {
  ...
  static _addDefaultAttributes() {
    ...
    const existingAttributes = nutils.clone(this.rawAttributes);
    this.rawAttributes = {};
  ...
  static init(attributes, options) { // testhint options:none
    ...
    options = Utils.merge(nutils.cloneDeep(globalOptions.define), options);
  ...
  static refreshAttributes() {
    ...
    _.each(['get', 'set'], type => {
      ...
      const funcs = nutils.clone(_.isObject(this.options[opt]) ? this.options[opt] : {});
  ...
  static findOrBuild(options) {
    ...
    return this.findOne(options).then(instance => {
      if (instance === null) {
        values = nutils.clone(options.defaults) || {};
  ...
  static findCreateFind(options) {
    ...
    let values = nutils.clone(options.defaults) || {};
    if (_.isPlainObject(options.where)) {
      values = Utils.defaults(values, options.where);
    }
  ...
  static bulkCreate(records, options) {
    ...
    return Promise.try(() => {
      ...
      if (options.validate) {
        ...
        const validateOptions = nutils.clone(options);
      ...
      if (options.individualHooks) {
        ...
        return Promise.map(instances, instance => {
          const individualOptions = nutils.clone(options);
  ...
  */
  ...
  _initValues(values, options) {
    ...
    values = values && nutils.clone(values) || {};
    ...
    if (options.isNewRecord) {
      ...
      if (this.constructor._hasDefaultValues) {
        defaults = _.mapValues(this.constructor._defaultValues, valueFn => {
          const value = valueFn();
          return value && value instanceof Utils.SequelizeMethod ? value : nutils.cloneDeep(value);
        });
      }
  ...
  set(key, value, options) { // testhint options:none
    ...
    if (typeof key === 'object' && key !== null) {
      ...
      if (options.raw && !(this._options && this._options.include) && !(options && options.attributes) && !this.constructor._hasBooleanAttributes && !this.constructor._hasDateAttributes) {
        ...
        this._previousDataValues = nutils.clone(this.dataValues);
      } else {
        // Loop and call set
        if (options.attributes) {
